...
void process_script_file(const char *file) {
    script_buffer = (char *) malloc(32768);
    ...
    if(EUROverbose) {
...
#if 0
        for(unsigned int i = 0; i < ImageXcount; ++i) {
            printf("%d\n", Image_Xcoords[i]);
        }
        printf("\n");
#endif
        ...
        printf("Clubcount  =  %d\n",Clubcount);
#if 0
        for(unsigned int i = 0; i < Clubcount; ++i) {
            printf("(%d) %s\n", i, Clubnames[i]);
        }
        printf("\n");
#endif
...
#if 0
        for(unsigned int i = 0; i < Speechcount; ++i) {
            printf("%s\n", SPEECH_identifier[i]);
        }
        printf("\n");
#endif
...
void ConvertLine(char *buf) {
    ...
    while (variable != NO_INSTRUCTION) {
        if (variable == IMAGE_xCOORD) {
            Image_Xcoords[ImageXcount] = GetVariableValue(buf);
            ImageXcount++;
        }

        if (variable == IMAGE_yCOORD) {
            Image_Ycoords[ImageYcount] = GetVariableValue(buf);
            ImageYcount++;
        }

        if (variable == IMAGE_WIDTH) {
            Image_Widths[ImageWcount] = GetVariableValue(buf);
            ImageWcount++;
        }

        if (variable == IMAGE_HEIGHT) {
            Image_Heights[ImageHcount] = GetVariableValue(buf);
            ImageHcount++;
        }

        if (variable == IMAGE_PAGE) {
            Image_Pages[Pagecount] = (char) GetVariableValue(buf);
            Pagecount++;
        }
...
//			{ GetAscCharacter(buf); LowerCasecount++;}
        ...
        if (variable == DIAL_TYPE) {
            LegalDial[Dialcount] = (unsigned char) GetCharacter(buf);
            Dialcount++;
        }

        if (variable == FORMATION_TYPE) {
            FormationInfo[Formationcount] = GetFormationType(buf);
            Formationcount++;
        }
        ...
        if (variable == PLAYERS_POSITION) {
            PlayerPosns[Posncount] = GetPositionType(buf);
            Posncount++;
        }

        if (variable == CAPS_TYPE) {
            PlayersCaps[Capscount] = GetFormationType(buf);
            Capscount++;
        }

        if (variable == GOALS_TYPE) {
            PlayersGoals[Goalscount] = GetFormationType(buf);
            Goalscount++;
        }
        ...
        if (variable == PACE_TYPE) {
            Pace[Pacecount] = (char) GetVariableValue(buf);
            Pacecount++;
        }
        if (variable == POWER_TYPE) {
            Power[Powercount] = (char) GetVariableValue(buf);
            Powercount++;
        }
        if (variable == CONTROL_TYPE) {
            Control[Controlcount] = (char) GetVariableValue(buf);
            Controlcount++;
        }
        if (variable == FLAIR_TYPE) {
            Flair[Flaircount] = (char) GetVariableValue(buf);
            Flaircount++;
        }
        if (variable == VISION_TYPE) {
            Vision[Visioncount] = (char) GetVariableValue(buf);
            Visioncount++;
        }
        if (variable == ACCURACY_TYPE) {
            Accuracy[Accuracycount] = (char) GetVariableValue(buf);
            Accuracycount++;
        }
        if (variable == STAMINA_TYPE) {
            Stamina[Staminacount] = (char) GetVariableValue(buf);
            Staminacount++;
        }
        if (variable == DISCIPLINE_TYPE) {
            Discipline[Disciplinecount] = (char) GetVariableValue(buf);
            Disciplinecount++;
        }
        if (variable == FITNESS_TYPE) {
            Fitness[Fitnesscount] = (char) GetVariableValue(buf);
        ...
        if (variable == MODEM_INIT_TYPE) {
            ModemInitList[ModemInitcount] = (char) GetVariableValue(buf);
            ModemInitcount++;
        }
...
void GetTextString(char *buf)    //, int inst )
{
    ...
    while (CRflag != 0) {
        ...
        if (buf[inst] > 31 && buf[inst] != '/' && buf[inst] != '"' && CRflag == 1) {
            TextStrings[StringOffset] = (BYTE) buf[inst];
            StringOffset++;
        }
        if (buf[inst + 1] > 31 && buf[inst] == '/' && CRflag == 1) {
            TextStrings[StringOffset] = (BYTE) buf[inst + 1];
            StringOffset++;
            inst++;
        }
...
short GetClubString(char *buf) {
    ...
    if (ClubFound == NOT_FOUND) {
        ...
        for (char t = 0; t < (strlen(PlayerClub)); t++) {
            TextStrings[StringOffset] = (BYTE) PlayerClub[t];
            StringOffset++;
        }
...
void Getvariable(char *buf) {
    ...
    while (buf[inst] != 0 && variable == NO_INSTRUCTION) {
        if (buf[inst + 1] == ':' && buf[inst - 1] <= ' ') {
            if (buf[inst] == 'x') {
                variable = IMAGE_xCOORD;
                inst += 2;
            }
            if (buf[inst] == 'y') {
                variable = IMAGE_yCOORD;
                inst += 2;
            }
            if (buf[inst] == 'w') {
        ...
        if (buf[inst + 4] == ':' || buf[inst + 3] == ':' || buf[inst + 5] == ':') {
            ...
            if (buf[inst] == 'd' && buf[inst + 1] == 'r' && buf[inst + 2] == 'a' && buf[inst + 3] == 'w') {
                variable = GROUP_DRAW_TYPE;
                inst += 5;
                break;
            }
            if (buf[inst] == 'c' && buf[inst + 1] == 'l' && buf[inst + 2] == 'u' && buf[inst + 3] == 'b' &&
                Datecount < 320) {
                variable = PLAYERS_CLUB_NAME;
                inst += 5;
                break;
            }
            if (buf[inst] == 'p' && buf[inst + 1] == 'o' && buf[inst + 2] == 's' && buf[inst + 3] == 'n' &&
                Posncount < 320) {
                variable = PLAYERS_POSITION;
                inst += 5;
                break;
            }
            if (buf[inst] == 'c' && buf[inst + 1] == 'a' && buf[inst + 2] == 'p' && buf[inst + 3] == 's' &&
                Capscount < 320) {
                variable = CAPS_TYPE;
                inst += 5;
                break;
            }
            if (buf[inst] == 'g' && buf[inst + 1] == 'o' && buf[inst + 2] == 'a' && buf[inst + 3] == 'l' &&
                Goalscount < 320) {
                variable = GOALS_TYPE;
                inst += 5;
                break;
            }
            if (buf[inst] == 'p' && buf[inst + 1] == 'a' && buf[inst + 2] == 'c' && buf[inst + 3] == 'e' &&
                Pacecount < 320) {
                variable = PACE_TYPE;
                inst += 5;
                break;
            ...
            if (buf[inst] == 'c' && buf[inst + 1] == 'n' && buf[inst + 2] == 't' && buf[inst + 3] == 'l' &&
                Controlcount < 320) {
                variable = CONTROL_TYPE;
                inst += 5;
                break;
            }
            if (buf[inst] == 'f' && buf[inst + 1] == 'l' && buf[inst + 2] == 'a' && buf[inst + 3] == 'r' &&
                Flaircount < 320) {
                variable = FLAIR_TYPE;
                inst += 5;
                break;
            }
            if (buf[inst] == 'v' && buf[inst + 1] == 'i' && buf[inst + 2] == 's' && buf[inst + 3] == 'n' &&
                Visioncount < 320) {
                variable = VISION_TYPE;
                inst += 5;
                break;
            }
            if (buf[inst] == 'a' && buf[inst + 1] == 'c' && buf[inst + 2] == 'c' && Accuracycount < 320) {
                variable = ACCURACY_TYPE;
                inst += 4;
                break;
            }
            if (buf[inst] == 's' && buf[inst + 1] == 't' && buf[inst + 2] == 'a' && buf[inst + 3] == 'm' &&
                Staminacount < 320) {
                variable = STAMINA_TYPE;
                inst += 5;
                break;
            }
            if (buf[inst] == 'd' && buf[inst + 1] == 'i' && buf[inst + 2] == 's' && buf[inst + 3] == 'c' &&
                Disciplinecount < 320) {
                variable = DISCIPLINE_TYPE;
                inst += 5;
                break;
            }
            if (buf[inst] == 'f' && buf[inst + 1] == 'i' && buf[inst + 2] == 't' && Fitnesscount < 320) {
                variable = FITNESS_TYPE;
                inst += 4;
                break;
            }
            ...
            if (buf[inst] == 'n' && buf[inst + 1] == 'a' && buf[inst + 2] == 'm' && buf[inst + 3] == 'e' &&
                Speechcount < 320) {
                variable = SPEECH_ID_STRING;
                inst += 5;
                break;
            }
