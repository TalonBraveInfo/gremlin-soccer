...
extern int draw_string(buff_info *buff, int font_no, int x, int y, const char *string, unsigned char col, short just);
extern int winrange;
...
extern unsigned char *readrawfile(const char *filename, unsigned char *address);
...
{
	int vesas[]={0x101,-1};
	unsigned char *scr=(unsigned char *)malloc(640*480);
...
// Initialise Penalty Shoot-out...
	...
	if (replay_seconds)
	{
		replay_tab=(plyrdat *)malloc(replay_seconds*ncb*20*sizeof(plyrdat));
		if (replay_tab==NULL)
		{
