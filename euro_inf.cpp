...
void DisplayPlayerStats() {
    ...
    short Plyr = (short) ((Team * 20) + ViewPlyrStats);
    short PlyrIndex = SquadInfo[Plyr];
    short ActPlyrIx = (short) ((Team * 20) + PlyrIndex);

    short tpace = (short) (Pace[ActPlyrIx] - (50.0 / 100.0) * (100 - Fitness[ActPlyrIx]));
    short tacc = (short) (Accuracy[ActPlyrIx] - (10.0 / 100.0) * (100 - Fitness[ActPlyrIx]));
    short tflair = (short) (Flair[ActPlyrIx] - (10.0 / 100.0) * (100 - Fitness[ActPlyrIx]));
    short tstam = (short) (Stamina[ActPlyrIx] - (50.0 / 100.0) * (100 - Fitness[ActPlyrIx]));
    short tpow = (short) (Power[ActPlyrIx] - (50.0 / 100.0) * (100 - Fitness[ActPlyrIx]));
    ...
    sprintf(NumberString, "%d", PlayersCaps[ActPlyrIx]);
    ...
    sprintf(NumberString, "%d", PlayersGoals[ActPlyrIx]);
    ...
    stat = (short) (((75 * Pace[ActPlyrIx]) / 100) +
                    ((25 * Stamina[ActPlyrIx]) / 100));

    Image_Widths[PURP_FILL + Dull] = (int) (stat * 1.14);
    ...
    stat = (short) (((60 * Power[ActPlyrIx]) / 100) +
    ...
    stat = (short) (((60 * tpow) / 100) +
                    ((40 * Discipline[ActPlyrIx]) / 100));

    Image_Widths[PURP_FILL + Bright] = (int) (stat * 1.14);
...
void DisplayTeamStats() {
    ...
    for (char s = 0; s < 3; s++) {
        ...
        DisplayString(
                426 - (PixelLengthOfString(GetPLAYERSname(StarPlayers[(EUROstatTeam * 3) + s], EUROstatTeam, FULL_NAME),
...
void DoTeamStats(char MENU, signed char BOX, short xposn, short yposn) {
    if (MENU == TEAM_STATS && LogicState == RUN_FRONTEND) {
        if (BOX == RETN_TSTATS_BOX && ButtonState != 0) {
            if (ButtonState == LEFT_BUTTON)
                Animation_table[ReturnTeamHandle].AnimationSpeed = -1.25f;
            else
                Animation_table[ReturnTeamHandle].AnimationSpeed = 1.25f;
        ...
        if (BOX == OPPONENT_TEAM_BOX && ButtonState != 0) {
            if (ButtonState == LEFT_BUTTON) {
                Animation_table[ViewTeamHandle].AnimationSpeed = -1.25f;
                EUROstatTeam++;
            } else {
                Animation_table[ViewTeamHandle].AnimationSpeed = 1.25f;
                EUROstatTeam--;
