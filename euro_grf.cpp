...
void spriteareainccopy(int copyWidth, BYTE *dest,
                       BYTE *srce) {                            // if a pixel falls within 'pmin' & 'pmax' then
...
void DrawIMAGE(pseudo_info *pseudo, texture_info *texture, int image_no, int xpos, int ypos, signed char box,
               copyfunctiontype copyfunction) {
...
void CopyFromBACKtoPSEUDObuffer(pseudo_info *pseudo, pseudo_info *background, int xpos, int ypos, short width,
                                short height) {
...
void CreateFilter(BYTE *FilterTable, char fr, char fg, char fb, float mr, float mg, float mb) {
    ...
    for (x = 0; x < 768; x += 3) {
        l2 = sqrt(PaletteBuffer[x] * PaletteBuffer[x] + PaletteBuffer[x + 1] *
                                                        PaletteBuffer[x + 1] +
                  PaletteBuffer[x + 2] * PaletteBuffer[x + 2]);
...
char *GetTEAMname(char team, char maxlen) {
    ...
    char NAMEoffset = 0;
    char x;
    for (x = 0; x < Searchlen; x++) { StringBuffer[x] = *(Name + x); }
    StringBuffer[x] = 0;
        ...
        if (string[soff] == '?') {
            ...
            if (!strcmp(Templete, "WINPENS")) {
                if (Results[flagA].HomePens > Results[flagA].AwayPens)
                    sprintf(Build, "%d", (int)Results[flagA].HomePens);
                else
                    sprintf(Build, "%d", (int)Results[flagA].AwayPens);
            }
