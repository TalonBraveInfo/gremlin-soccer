...
#ifdef IMPLEMENT_ME
    char len = (char) strlen(&string[0]);
    char u;
    for (u = 0; u < len; u++) { StringBuffer[u] = ToUpper[*(string + u)]; }
    StringBuffer[u] = 0;
...
void
DisplayTextInputCursor(int xpos, short CursorPosn, int ypos, char *string, int font, int colour, pseudo_info *pseudo,
...
short
AddAnimation(int xposn, int yposn, float StartFrame, float EndFrame, float Speed, signed char Box, signed char Flags) {
    short handle = 0;
...
void InitialiseMenuPage(char MENU) {
    switch (MENU) {
        ...
        case (SQUAD_SELECT):
            LoadTactics(((int) FormationInfo[Team]), EuroDATfile, &TeamTactics[0][0][0]);
            DisplaySquad();
            DisplayFormation(0);
...
void StartingEURO_96(char gametype) {
    ...
    if (EUROverbose != 0) {
        printf("Calling Set_Comm_Type with comm type: %d\n", EUROcommTYPE);
        fflush(stdout);
    }
#ifdef IMPLEMENT_ME
...
#endif
    if (EUROverbose != 0) {
        printf("Calling Start_Network_Game.\n");
        fflush(stdout);
    }

    if (Start_Network_Game() != 0) {
        if (gametype == EURO_wireplay) {
            printf("ERROR, cannot initialise WIREPLAY game.\n");
            fflush(stdout);
        }

        if (gametype == EURO_network_game) {
            printf("ERROR, cannot initialise NETWORK game.\n");
            fflush(stdout);
        }

        if (gametype == EURO_serial_game) {
            printf("ERROR, cannot initialise SERIAL game.\n");
            fflush(stdout);
        }

        if (gametype == EURO_modem_game) {
            printf("ERROR, cannot initialise MODEM game.\n");
            fflush(stdout);
        }
