...
extern void options(buff_info *buffer,datapt viewx,datapt viewy,datapt viewz,datapt targx,datapt targy, datapt targz,datapt vdist,plyrdat *plyrtb,datapt *xyz_ptr,word *ref_ptr,int,int);
extern void substitution(buff_info *buffer,datapt viewx,datapt viewy,datapt viewz,datapt targx,datapt targy, datapt targz,datapt vdist,plyrdat *plyrtb,datapt *xyz_ptr,word *ref_ptr,int);
extern void switch_coords();
extern int icth,isth;
...
volatile user_vectors_i users_dir_i[512][MAX_NET_USERS];	// Declared in network.c
short wConsoleNode;
int norm_font_col=31;
int counter=0;
int deb_1;
int deb_2;
int deb_1a;
...
static void ProcessOverlays( void);
...
char info_str1[32];
	...
	sprintf(info_str1,"Fps %02d", FramesPerSecond);
	draw_string_center(40, 10, info_str1,&main_buff);
}
#endif
...
void display_message(char *t)
{
	new_text=REAL_SPEED*3;
	text_mess=t;
}
...
{
char cams[]={ 1, 2, 3, 7, 5, 8, 9, 6, 0, 0, 0, 0, 0, 0, 4 };
	...
	if( camera <15)
		display_message( GetText( cams[c], textCamera ));
}
...
#if USERAVEENGINE!=YES
	if ( match_info.vidi && !vidi_anim)		// Vidi screen on...
		{
		render3d( &vidi_buff, cntspot_x, cur_camz, pitch_wid+80, eff_ballx, eff_ballz, eff_bally, scale, pos, &spr_coord_tab[0], &spr_ref_tab[0]);
		vidi_buff.dump(0,0);
		}
#endif
	render3d( &main_buff, cur_camx, cur_camz, cur_camy, cur_tx, cur_tz, cur_ty, scale, pos, &spr_coord_tab[0], &spr_ref_tab[0]);
...
// Substitution Menu...
			...
			substitution(&main_buff,cur_camx,cur_camz,cur_camy,cur_tx,cur_tz,cur_ty,scale,pos,&spr_coord_tab[0],&spr_ref_tab[0],opt_team);
			if (!player_being_subbed)
				camera=sub_camera;
			opt_user=FALSE;
			}
		else
			options( &main_buff, cur_camx, cur_camz, cur_camy, cur_tx, cur_tz, cur_ty, scale, pos, &spr_coord_tab[0], &spr_ref_tab[0], --opt_user, opt_team);
	...
	if (new_text)
		draw_string_center(main_buff.clip_wid>>1,16,text_mess,&main_buff);
...
char t;
		...
		t = teams[last_touch-1].shirt;
		name[0]=(t/10)+48; if(name[0]==48) name[0]=32;
		name[1]=(t%10)+48; name[2]=0;

		if (!c)
			n=game_data[setup.team_a].players[p-1].name;
		else
			n=game_data[setup.team_b].players[p-1].name;

		strcat(name,". "); strcat(name,n);

		if (!c)
			{
			if(!match_info.res_toggle)
				draw_string( &main_buff, 1, 5, 1, name, TEAM1COL, 0);
			else
				draw_string( &main_buff, 2, 5, 1, name, TEAM1COL, 0);
			}
		else
			{
			if(!match_info.res_toggle)
				draw_string( &main_buff, 1, main_buff.clip_wid-5, 1, name, TEAM2COL, 1);
			else
				draw_string( &main_buff, 2, main_buff.clip_wid-5, 1, name, TEAM2COL, 1);
			}
		}
