...
{
	if (plr!=KP_A && plr!=KP_B && match_info.bookings)
	{
...
// Shall we send him off?
	...
	if (direct && (penalty(fouler)))
	{
		...
		init_speech(players+1,SAY_PENALTY,0); // Joolz
		game_action=2;		// gather outside box!
		if (fouler>11)
// Team A P.K.
		{
...
// Not on the ball..(INDIRECT)
				init_speech(players+1,SAY_OFFSIDE,0); // Joolz
...
// On the ball..(DIRECT)
			{
				...
				init_speech(players+1,SAY_FOUL,0 ); // Joolz
...
{	
	...
	if (!just_scored && match_info.freekicks && ((seed<d) || (see)))
	{
...
// Ball is free...play temporary advantage...
				{
					play_advantage=TRUE;
					init_speech(players+1,SAY_ADV,0); // Joolz
				}
				else
					if (ball_poss<12)
						punish_foul(fouler,direct_fk);
...
// Referee go to spot behind incident...
	refs[0].act=1;
	refs[0].target=0;	// Ball!
	refs[0].goto_x=ballx-(x*BESIDE_BALL*16/d);
	refs[0].goto_y=bally-(y*BESIDE_BALL*16/d);
...
inline void inc_fouls(int p)
{
	teams[p-1].tm_fouls++;		// add direct F.K. to foul list.
}

/*****************************************************************************
*****************************************************************************/

inline void inc_pass(int p)
{
...
inline void inc_inj(int p,int i)
{
...
inline void inc_red(int p)
{
	teams[p-1].tm_book+=2;		// Red card.
}

/******************************************************************************
*****************************************************************************/

inline void inc_shot(int p)
{
...
inline void inc_save(int p)
{
	teams[p-1].tm_saved++;		// Goals saved.
}
...
// Centre.
		...
		case(CENTRE_B):
			...
			play_fx(PS_SHORTWHISTLE,refs[0].x,refs[0].y);
			init_centre();
//			if( match_time.min==45 || match_time.min==90 || match_time.min==105)
			setp_wait_cnt=MAX_SETP_WAIT;
			break;
