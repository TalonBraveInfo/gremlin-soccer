...
short	run_button_timer[4]={0,0,0,0};
short	run_button_timer_Tmp[4]={0,0,0,0};
long	RunTimeDel[4]={0,0,0,0};
...
// Keeper with ball in his hands...(slow)
					...
					// With Ball!!!
					a=(((float)pitch_len)/((((24.0 - (run_button_timer[user_controlled-1]/3) ))-(((float)(p->tm_rate)/64)*4))*20))+1;
					//slPrintFX(a, slLocate(1,8));


					}					
					else{

					// Run half speed of run speed
...
// Without Ball!!!
			...
			if (user_controlled && run_button_timer[user_controlled-1] ){	//uf2)
					a=((float)pitch_len)/((((18 - (run_button_timer[user_controlled-1]/3) ))-(((float)(p->tm_rate)/64)*4))*20);
					//slPrintFX(a, slLocate(1,8));	
			}			
			else{
...
float aactual_spd(match_player *p)
{
	float a;
...
// Arcade Mode!!!
		...
		{
			teams[i].tm_rate=30+(avg/2);
			teams[i].tm_pow=avg;
			teams[i].tm_cont=avg;
			teams[i].tm_flair=avg;
			teams[i].tm_vis=avg;
			teams[i].tm_ac=avg;
			teams[i].tm_stam=avg;
			teams[i].tm_disc=avg;
		...
		avg=50+(1.4*(COUNTRIES-rank-1));				// 50 - 110
...
// Simulation Mode!!!
		for (i=0; i<(players/2); i++)
		{
			teams[i].tm_rate=game_data[setup.team_a].players[i].pace+28;
			teams[i].tm_pow=game_data[setup.team_a].players[i].power+28;
			teams[i].tm_cont=game_data[setup.team_a].players[i].control+28;
			teams[i].tm_flair=game_data[setup.team_a].players[i].flair+28;
			teams[i].tm_vis=game_data[setup.team_a].players[i].vision+28;
			teams[i].tm_ac=game_data[setup.team_a].players[i].accuracy+28;
			teams[i].tm_stam=game_data[setup.team_a].players[i].stamina+28;
			teams[i].tm_disc=game_data[setup.team_a].players[i].discipline+28;
		}

		for (i=11; i<(players); i++)
		{
			teams[i].tm_rate=game_data[setup.team_b].players[i].pace+28;
			teams[i].tm_pow=game_data[setup.team_b].players[i].power+28;
			teams[i].tm_cont=game_data[setup.team_b].players[i].control+28;
			teams[i].tm_flair=game_data[setup.team_b].players[i].flair+28;
			teams[i].tm_vis=game_data[setup.team_b].players[i].vision+28;
			teams[i].tm_ac=game_data[setup.team_b].players[i].accuracy+28;
			teams[i].tm_stam=game_data[setup.team_b].players[i].stamina+28;
			teams[i].tm_disc=game_data[setup.team_b].players[i].discipline+28;
		}
	}
...
// Not started kick yet...
			...
			if (seed>(108+((128-player->tm_flair)/8)))
			{
// Decides to kick...
				AF_randomize(__FILE__,__LINE__);
				if (seed>player->tm_flair)
				{
...
void init_tackle_act(match_player *player,float tx,float ty)
{
	...
	if (a>=cos(MAX_TURN))
//	if (!shall_i_turn(player,x,y,FALSE))
	{
...
// Face Ball...
	...
	if ((an>=MC_BFOOTBL && an<=MC_BBODYCR) || (an>=MC_CFOOTBL && an<=MC_CBODYCR))
	{
...
//				else
					if (player->int_move==I_HEADER)
					{
						...
						{
							player->int_cnt=-1;
							init_jump_act(player,FALSE, FALSE);	// Jump straight up!
						}
					}
					else
					{
...
void user_run(match_player *player)
{
	...
	if ((px<-stadlist[setup.stadium].st_l+prat && ux<0)
		 || (px>pitch_len+stadlist[setup.stadium].st_l-prat && ux>0))
	{
		ux=0;
		if (ABS(uy)<0.05)
			init_stand_act(player);
	...
	if ((py<-stadlist[setup.stadium].st_w+prat && uy<0)
		|| (py>pitch_wid+stadlist[setup.stadium].st_w-prat && uy>0))
	{
		uy=0;
		if (ABS(ux)<0.05)
			init_stand_act(player);
...
// Update current movement!
			if (force/16>seed && (!(seed&3) || p2->tm_act==SAVE_ACT))
			{
				...
				{
					man_down=TRUE;
					init_foul(p2->tm_player,TRUE, FALSE);	// pass no. of fouler.
				}
				else
				{
					p1->tm_x+=(ef_xd);
...
// Update current movement!
			if (force/16>seed && (!(seed&3) || p2->tm_act==SAVE_ACT))
			{
				...
				{
					man_down=TRUE;
					init_foul(p1->tm_player,TRUE, FALSE);	// pass no. of fouler.
				}
			}
			else
			{
...
// He is NOT bound up in animation!!!
					...
					if (!just_thrown || !ball_limbo_on)			// Joolz (Saturn)
