...
// Feet level...
	{
		if (bp<=SAVE_AFOOTB_TIME/keeper_spd && !keep_dive)
		{
			...
			if (factor>KP_SAVE_MARGIN && !forced)
			{
// Ball is too fast!
				factor=KP_SAVE_MARGIN;
...
void go_to_save_path(match_player *player,char forced)
{
...
int angle_of_vis(match_player *player,int x,int y,float d,char cross)
{
  	float a;
	float xd=player->tm_xdis;
	float yd=player->tm_ydis;
		...
		&& (!dead_ball_cnt) && (!just_scored))
	{
		...
		for (int i=1; i<=players; i++)
		{
			if (teams[i-1].tm_act<=TURN_ACT)
			{
				...
				{
					if (seed<(42+((player->tm_pow-teams[i-1].tm_pow)/3)))
					{
// Crunching tackle...
						init_fall(&teams[i-1]);
...
// Therefore a foul has been commited...
							man_down=calc_dist(player->go_txdis+teams[i-1].go_txdis,
													 player->go_tydis+teams[i-1].go_tydis);

							init_foul(player->tm_player,TRUE, FALSE);	// pass no. of fouler.
...
void make_pass(match_player *player,int p,char cross)
{
	kick_type=KT_PASS;
	float sx,sy,sz;
	cross_now=cross;
...
int pass_decide(match_player *player,char cross)
{
	char angle;
	int passee_tab[10][3];
	int passees=0;
...
int get_facing_opp_dir(match_player *player)
{
...
// I am close enough to hassle!
				{
					int d=get_facing_opp_dir(player);
