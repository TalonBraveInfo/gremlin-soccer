...
// Ball above player's head!
		...
		else
		{
			...
			else
			{
				if (!dead_ball_cnt)
				{
// If player is in a set-piece then no need to mave position of ball...
...
// ftime (-2 to -10)...
				ballx=bx+((bx-ballx)*(-2-player->tm_ftime)/8);
				bally=by+((by-bally)*(-2-player->tm_ftime)/8);
...
// At head and body height...
				...
				{
// Keeper save!
					if (player->tm_frm>=keeper_contact)
					{
