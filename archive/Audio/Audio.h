...
EXTERN int CrowdVolume;
EXTERN int PitchVolume;
EXTERN int CommentaryVolume;
EXTERN int MusicVolume;
EXTERN int MasterVolume;
EXTERN Boolean	 bMusicPlaying;
EXTERN short	 fMusicRefNum;
...
struct memstatus
	{
//	unsigned int ext;				// Running total for Extended memory allocations
//	unsigned int base;				// Running total for Base memory allocations
	unsigned int fail;				// Running total for Failed memory allocations
	};
...
typedef struct tagmemsam
	...
	int size;							// Length of the sample when in memory
...
typedef struct tagpitchsam
	...
	int freq;							// Frequency Divider (typically 0x8000 - half)
	unsigned short int flags;
	}pitchsam;
...
struct crowdsam
{
	int mood;					// Current mood of Crowd
	int tranmood;			// Mood in for transition to
	int mainhandle;			// Main sample handle of crowd sample
	int mainvol;				// Volume of main crowd sample
	int maindelta;			// Volume difference of main crowd sample
	int mergehandle;		// Sample handle of Merging Crowd Sample
	int mergevol;			// Volume of Merging Crowd Sample
	int mergedelta;			// Volume Difference of Merging Sample
	int volmax;				// Maximum volume crowd can reach
	int pan;					// Pan Position of Crowd Sample
	int pandelta;			// Pan Difference of Crowd Sample
	int ticks;				// Number of Ticks since last random event
	int nextevent;			// Number of Ticks to next event
...
EXTERN unsigned int InitMatchAudio(unsigned int seed);
EXTERN void UpdateVolumeLevels(void);

EXTERN pascal void MusicCompletion( SndChannelPtr sndChannel);
EXTERN int StartMusic(char *name,int volume);
EXTERN void StopMusic(void);
EXTERN int UpdateMusic(void);
EXTERN int CheckMusic(void);

EXTERN unsigned int LoadPitchSamples(void);
EXTERN void FreePitchSamples(void);
EXTERN int PlayPitchSample(int sam,short int amp,short int pan);

EXTERN void StartCrowd(int team);
EXTERN void StopAudio(void);
EXTERN int PlayCrowdSample(int sam);
EXTERN void UpdateCrowd(int mood,int team);

EXTERN unsigned int LoadTeamSamples(int team_num,int team);
EXTERN void FreeTeamSamples(int team);

EXTERN unsigned int LoadFrontEndSamples(void);
EXTERN void FreeFrontEndSamples(void);

EXTERN unsigned int LoadCommentarySamples(void);
...
EXTERN int PlayTeamSample(int player,int intonation);
...
EXTERN int PlayFrontEndSample(int sam,int amp);
...
int LoadSample(char *mad,char *name,memsam *bank,struct memstatus *mem);
