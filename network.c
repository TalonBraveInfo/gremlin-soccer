...
extern void JoyRead3(void *);

extern void JoyRead2(void *);

extern void JoyRead1(void *);
...
void ReadJoys(struct joy_struct *JoyData) {
    ...
    do {
        ...
        timecount++;
             ...
             timecount < JOY_TIMEOUT);        //this will cause time out during calibration if not connected!
    _enable();
...
void read_joy2(unsigned short *x, unsigned short *y, unsigned char *sw) {
    *x = (unsigned short) joy_pos.JoyBX;
    *y = (unsigned short) joy_pos.JoyBY;
    *sw = (unsigned char) (joy_pos.FireB1 | (joy_pos.FireB2 << 1));
}

void read_joystick(char j, unsigned short *x, unsigned short *y, unsigned char *sw) {
#ifdef IMPLEMENT_ME
    ReadJoys((joy_struct *)&joy_pos);
    if (!j)
        read_joy1(x, y, sw);
    else
        read_joy2(x, y, sw);
                    ...
                    if (wConsoleNode == master)                                        // If master...
                    {
                        if (start_multiple == network_on - 1)                    // If all data sent last frame
                        {
                            for (i = 0; i < MAX_NET_USERS; i++) {
                                users_dir_i[writeptr][i].x_f1 = users_dir_i[(writeptr - 1) &
                                                                            511][i].x_f1; // Copy all last input data for defaults
                        ...
                        if (active_nodes ==
                            network_on)                                // Only start transmitting when ready
                        {
                            if (start_multiple != network_on - 1 ||
                                !paused)        // Stop sending if paused, and block fully transmitted
                            {
                                send_multiple();
                                if (start_multiple ==
                                    network_on - 1)            // If block fully transmitted, move to next
                                {
                                    writeptr++;
                                    writeptr &= 511;
                                }
                    ...
                    } else                                // If slave...
                    {
                        ...
                        for (i = 0; i < network_on; i++)
                            if (users[i].type && users[i].control != NET_CTRL &&// If local human user,...
                                (users_dir_i[writeptr][i].x_f1 != users_dir_i[(writeptr - 1) & 511][i].x_f1 ||
                                 // and input has changed...
                                 users_dir_i[writeptr][i].y_f2 != users_dir_i[(writeptr - 1) & 511][i].y_f2))
                                send_single(
                                        i);                                                                                // then send to master
...
void transmitpausetoslaves(void) {
    ...
    if (network_on) {
        ...
        for (i = 0; i < network_on; i++)
            if (i != master)
                while (!hmiNETNOWSendData((PSTR) & header, sizeof(packet_header), (PSTR) & data, sizeof(packet_pause),
                                          i));
#endif
    }
}
...
void TransmitInit(void) {
    ...
    if (network_on) {
        ...
        while (!hmiNETNOWSendData((PSTR) & header, sizeof(packet_header), (PSTR) & info, sizeof(packet_info),
                                  _NETNOW_BROADCAST));
#endif
    }
}
