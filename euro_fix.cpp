...
void CalculateLeagueTables()
	{
		...
		if ( EUROverbose != 0 )
			...
			printf(" (%d)	  W: %d   D: %d   L: %d   Gf: %d   Ga: %d   Pts: %d       %s\n", 
						...
						LeagueTable[t].goalsFor, LeagueTable[t].goalsAgainst,
						LeagueTable[t].points,	GetTEXT((short) (TEAM_NMES + t)) );
			fflush(stdout);				
		}
	...
	for ( char h=0; h < 16 ; h++ )
		{
			DisplayString(
				...
				sprintf(ammount, "%d", LeagueTable[team].won);
				DisplayString( 
				WinPosnDEFN[h].xposn - 
				...
				sprintf(ammount, "%d", LeagueTable[team].drew);
				DisplayString( 
				...
				sprintf(ammount, "%d", LeagueTable[team].lost);
				DisplayString( 
                ...
                sprintf(ammount, "%d", LeagueTable[team].points);
				DisplayString( 
...
void DisplayLeagueFixtures( char Match )
	{		  
		...
		for ( char f=0; f < 8 ; f++ )
		{
			...
			strcat(ResText, " ");
            sprintf(EUROammount, "%d", (int)Results[Match].HomeScore);
			...
			strcat(ResText, " ");
            sprintf(EUROammount, "%d", (int)Results[Match].AwayScore);
			strcat(ResText, EUROammount);
		...
		DefaultdrawHighlight =  ControlOptionHighlight( BOX, DEFAULT_BOX,
			...
			if ( EuroReDrawCounter <= 0.0 )
			{
				...
				if ( EuroReDrawFlag == 21 )
				{
					...
					for ( char t=0; t < 16 ; t++ )	    	 
						{
						c		 = (char) (GroupDrawInfo[t] / 4);
						GroupDrawInfo[t] = (signed char) ((GroupDrawInfo[t] & 3) + (GroupVenues[c] * 4));
						}						
					EuroReDrawFlag++;
						     ...
						     GroupVenues[3] != venue )
						{
							...
							CopyFromBACKtoPSEUDObuffer(
						 	...
						 	DisplayString(
						 		  ...
						 		  ((PixelLengthOfString( GetTEXT((short) (VNUE_NMES + (venue * 2))),
						 		  SMALL_FONT )) /2),
							 	  ReDrawHeadPosnDEFN[v].yposn-7,
								  GetTEXT((short) (VNUE_NMES + (venue * 2))),
								  SMALL_FONT, 
								  15,
								  NO_BOX, 
						 	...
						 	DisplayString(
							 	  ReDrawHeadPosnDEFN[v].xposn-
							 	  ((PixelLengthOfString( GetTEXT((short) (VNUE_NMES + Birmingham + (venue * 2))),
							 	  SMALL_FONT )) /2),
							 	  ReDrawHeadPosnDEFN[v].yposn+8,
								  GetTEXT((short) (VNUE_NMES + Birmingham + (venue * 2))),
								  SMALL_FONT, 
								  15,
								  NO_BOX, 
								  &FrontendPseudoDEFN, &FrontendTextureDEFN  );
							...
							AddToDumpList(
									(short) (ReDrawHeadPosnDEFN[v].xposn - 84),
									(short) (ReDrawHeadPosnDEFN[v].yposn - 7),
								  168, 32 );
				...
				{
					venue	= (signed char) (GroupDrawInfo[England] / 4);
					EuroReDrawFlag	=	17;
					...
					AddToDumpList(
							(short) (ReDrawHeadPosnDEFN[venue].xposn - 84),
							(short) (ReDrawHeadPosnDEFN[venue].yposn - 7),
					  168, 32 );
				...
				if ( EuroReDrawFlag > 11 && EuroReDrawFlag < 16 )
				{
					...
					if ( (DrawnTeam == England ||		
					     ...
					     GroupDrawInfo[DrawnTeam] == -1 )
						{
							...
							AddToDumpList(
                                    (short) (ReDrawPosnDEFN[EuroReDrawFlag].xposn - 84),
                                    (short) ReDrawPosnDEFN[EuroReDrawFlag].yposn,
								168, 20 );
	...
	{		  
		...
		LoadPalette( BIN_TBUL_PAL+( 8*FindNextFixtureTeam( Fixtures[Match].AwayTeam ) ),
		 &EuroDATfile[0], (44+7+(14*f)) );
			...
			Outcome	= StringTemplete((short) (RSLT_TEXT + Results[Match].Message), Match );
